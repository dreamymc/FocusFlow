...
<template>
  <div
    class="bg-surface rounded-xl border border-border border-l-4 p-3.5 shadow-xs select-none transition-all duration-300"
    :class="[
      priorityBorderClass,
      readOnly
        ? 'cursor-default'
    ...
    <div class="flex items-center justify-between mt-3 pt-2 border-t border-border">
      <div class="flex items-center gap-2">
        ...
        <span
          ...
          :class="isOverdue(task.due_date)
            ? 'text-accent-red bg-accent-red/10 border-accent-red/20 font-semibold'
            : 'text-text-secondary bg-surface-3 border-border'"
        >
      ...
      <div v-if="task.assignees && task.assignees.length > 0" class="flex -space-x-1.5 overflow-hidden items-center">
        <div
          ...
          class="w-5.5 h-5.5 rounded-full bg-surface-3 text-text-secondary flex items-center justify-center font-semibold text-[9px] font-display border-2 border-surface shrink-0"
        ...
        <div
          ...
          class="w-5.5 h-5.5 rounded-full bg-surface text-text-muted flex items-center justify-center font-medium text-[8px] font-display border-2 border-surface shrink-0"
          :style="{ zIndex: 5 }"
